...
from bzt.utils import get_full_path, ensure_is_dict, PythonGenerator, FileReader, locust_variable_converter
from bzt.utils import shutdown_process, RequiredTool, dehumanize_time, guess_csv_dialect
...
class LocustIOExecutor(ScenarioExecutor, WidgetProvider, FileLister, HavingInstallableTools, SelfDiagnosable):
    ...
    def __copy_csv_data_sources(self):
        # scenario = self.get_scenario()
        source = self.scenario.get("csv-data-source")
        if source:
            source_path = self.engine.find_file(source["path"])
            print source_path
            self.engine.existing_artifact(source_path)
            print os.path.join(get_full_path(__file__, step_up=1), '../resources', 'csvFileReader.py')

            self.engine.existing_artifact(os.path.join(get_full_path(__file__, step_up=1), '../resources', 'csvFileReader.py'))
            # TODO Need a robust json parser.
            # self.engine.existing_artifact(os.path.join(get_full_path(__file__, step_up=1), '../resources', 'jsonParser.py'))
...
"""
    ...
    def build_source_code(self):
        ...
        sources = self.scenario.get("csv-data-source")
        if sources:
            LocustIOScriptBuilder.IMPORTS = LocustIOScriptBuilder.IMPORTS + "import csvFileReader"
        ...
        if sources:
            scenario_class.append(self.__gen_on_start_task(sources))
        # scenario_class.append(self.__gen_csv_datasources())
    ...
    def __gen_on_start_task(self, sources):
        if isinstance(sources, list):
            raise TaurusConfigError("csv-data-source '%s' is not a list" % sources)

        source_path = sources["path"]
        variables = sources["variables"]
        csv_headers = variables.split(',')
        options = sources["csv-options"]
        client = options.get('client')
        reading_order = options.get('order')

        task = self.gen_method_definition("on_start", ['self'])
        task.append(self.gen_statement('global rows'))
        task.append(self.gen_statement('global column_dict'))
        task.append(self.gen_statement("print 'client Hatched'"))
        task.append(self.gen_statement('rows, column_dict = csvFileReader.readCSVFile("{}", {})'.format(source_path, json.dumps(csv_headers))))
        task.append(self.gen_statement("print rows"))
    ...
    def __gen_task(self):
        ...
        with_csv_reader = False
        sources = self.scenario.get("csv-data-source")
        if sources:
            options = sources.get('csv-options')
            #  Each locust client is reading all the lines or one line in csv file
            client_csv_read_pattern =  options.get('client')
            if client_csv_read_pattern.lower() == 'one':
                csv_headers = sources.get('variables').split(',')
                for csv_header in csv_headers:
                    print csv_header
                    task.append(self.gen_statement("{test} = row[column_dict['{test}']]".format(test=csv_header)))
            else:
                with_csv_reader = True
                task.append(self.gen_statement("for row in rows:"))
                csv_headers = sources.get('variables').split(',')
                for csv_header in csv_headers:
                    print csv_header
                    task.append(
                        self.gen_statement("{test} = row[column_dict['{test}']]".format(test=csv_header), indent=12))
        ...
        for req in self.scenario.get_requests():
            ...
            self.__gen_check(method, req, task, dehumanize_time(timeout), global_headers, with_csv_reader)
    ...
    def __get_params_line(req, timeout, headers):
        ...
        param_dict = {'url': '%s' % locust_variable_converter(req.url), 'timeout': timeout}
        if req.body:
        ...
        if headers:
            ...
            for element in headers:
                header_element = locust_variable_converter(headers[element])
                param_dict['headers'][element] = '%s' % header_element

            header_keys = (list(param_dict['headers'].keys()))
            param_dict['headers'] = '{%s}' % ', '.join(['"%s":''%s' % (key, param_dict['headers'][key]) for key in header_keys])
    ...
    def __gen_check(self, method, req, task, timeout, global_headers, with_csv_reader):
        csv_indent = 0
        if with_csv_reader:
            csv_indent = 4
        extract_response = req.config.get("extract-response")
        assertions = req.config.get("assert", [])
        first_assert = True
        if assertions or extract_response:
        ...
        if req.headers:
            ...
            task.append(
                self.gen_statement(statement % (method, self.__get_params_line(req, timeout, headers)), indent=8 + csv_indent))
        ...
        for idx, assertion in enumerate(assertions):
            ...
            self.__gen_assertion(task, assertion, first_assert, csv_indent)
            first_assert = False

        if assertions:
            task.append(self.gen_statement('else:', indent=12 + csv_indent))
            task.append(self.gen_statement('response.success()', indent=16 + csv_indent))

        self.log.debug("Extracting the data from the response.....")
        if not extract_response:
            self.log.debug("Dont have any response!!!!")
        else:
            statement = 'output = json.loads(response.content)'
            task.append(self.gen_statement(statement, indent=12 + csv_indent))
            for key in extract_response.keys():
                statement = key + ' = output["' + extract_response[key] + '"]'
                task.append(self.gen_statement(statement, indent=12 + csv_indent))
                task.append(self.gen_statement('print ' + key, indent=12+ csv_indent))

    def __gen_assertion(self, task, assertion, is_first, csv_indent):
        ...
        task.append(self.gen_statement(statement, indent=16 + csv_indent))
