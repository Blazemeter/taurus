...
def data_source_name(filename):
    filename = os.path.basename(filename)
    pure_name = filename.split('.')[0]
    var_name = normalize_method_name(pure_name) + '_feeder'
    return var_name
...
class ApiritifScriptGenerator(PythonGenerator):
    ...
    def __init__(self, scenario, label, file_finder, parent_log):
...
""").body)
    ...
    def gen_data_sources(self):
        stmts = []
        for data_source in self.scenario.get('data-sources', []):
            resolved_filename = self.file_finder(data_source)
            var_name = data_source_name(data_source)
            stmts.extend([
                ast.Assign(
                    targets=[
                    ...
                    value=ast.Call(
                        func=ast.Attribute(value=ast.Attribute(value=ast.Name(id="apiritif", ctx=ast.Load()),
                                                               attr="feeders",
                                                               ctx=ast.Load()),
                                           attr="CSVFeeder",
                                           ctx=ast.Load()),
                        args=[ast.Str(s=resolved_filename), ast.Name(id='vars', ctx=ast.Load())],
                        keywords=[],
                        starargs=None,
                        kwargs=None
                    )
    ...
    def gen_data_sources_next(self):
        stmts = []
        for data_source in self.scenario.get('data-sources', []):
            var_name = data_source_name(data_source)
            stmts.extend([
                ast.Expr(value=ast.Call(func=ast.Attribute(value=ast.Name(id=var_name), attr="next", ctx=ast.Load()),
                                        args=[],
                                        keywords=[],
                                        starargs=None,
                                        kwargs=None)),
    ...
    def gen_global_vars(self):
        ...
        stmts.extend(self.gen_data_sources())
        return stmts
