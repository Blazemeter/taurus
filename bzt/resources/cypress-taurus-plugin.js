...
function usage() {
    process.stdout.write("Taurus Cypress Plugin\n");
    process.stdout.write("\n");
    process.stdout.write("Usage:\n");
    process.stdout.write("--spec FILE - path to test script\n");
...
function loopCypress(config, cypressConfig, iterations, startTime, done) {
    ...
    var engine = prepareCypress(config, cypressConfig);
    engine.run(function() {
        loopCypress(config, cypressConfig, iterations + 1, startTime, done);
    });
}
...
function runCypress() {
    ...
    var done = function() {
        ...
        setInterval(function() {
            var totalTests = cypressConfig.reporterOptions.totalTests,
                itemsWritten = cypressConfig.reporterOptions.itemsWritten;
    ...
    loopCypress(config, cypressConfig, 0, epoch(), done);
}
