...
Available settings are:
 ...
 - `env` - environment variables to set for Taurus, useful with [evaluating feature](EnvironmentVariableAccess). Setting environment variable to `null` makes it to delete variable, if one is set. Special `TAURUS\_ARTIFACTS\_DIR` variable is set by Taurus, pointing onto artifacts dir location.
