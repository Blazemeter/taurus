...
class TestConverter(BZTestCase):
    ...
    def same_yaml(self, file1, file2=None):
        ...
        yml1 = full_load(open(file1).read())
        yml2 = full_load(open(file2).read())
        return yml1 == yml2
    ...
    def test_copy_global_csv_dataset(self):
        ...
        self.obj.process()
        yml = full_load(open(self.obj.dst_file).read())
    ...
    def test_json_body(self):
        ...
        for label in targets:
            self.assertTrue(not (
                        not (bodies[label] is None and targets[label] is None) and not isinstance(bodies[label],
                                                                                                  targets[label])))
