...
class TestLocSc(unittest.TestCase):
    ...
    def _1_(self):
        with apiritif.smart_transaction('/'):
            ...
            var_loc_chain = self.mng.get_locator([{'xpath': '/html/body/div[3]/h2'}])
            ActionChains(self.driver).double_click(self.driver.find_element(
                var_loc_chain[0],
                var_loc_chain[1])).perform()

            var_loc_chain = self.mng.get_locator([{'xpath': '/html/body/div[3]/form/select[1]'}])
            ActionChains(self.driver).click_and_hold(self.driver.find_element(
                var_loc_chain[0],
                var_loc_chain[1])).perform()

            var_loc_chain = self.mng.get_locator([{'xpath': '/html/body/div[3]/form/select[1]/option[6]'}])
            ActionChains(self.driver).release(self.driver.find_element(
                var_loc_chain[0],
                var_loc_chain[1])).perform()

            var_loc_select = self.mng.get_locator([{'name': 'toPort'}])
            Select(self.driver.find_element(
                var_loc_select[0],
                var_loc_select[1])).select_by_visible_text('London')

            var_loc_keys = self.mng.get_locator([{'css': 'body input.btn.btn-primary'}])
            self.driver.find_element(
                var_loc_keys[0],
                var_loc_keys[1]).send_keys(Keys.ENTER)

            var_loc_as = self.mng.get_locator([{'id': 'address'}])
            self.assertEqual(self.driver.find_element(
                var_loc_as[0],
                var_loc_as[1]).get_attribute('value').strip(), '123 Beautiful st.'.strip())

            var_loc_as = self.mng.get_locator([{'xpath': '/html/body/div[2]/form/div[1]/label'}])
            self.assertEqual(self.driver.find_element(
                var_loc_as[0],
            ...
            var_loc_keys = self.mng.get_locator([{'xpath': '//div[3]/form/select[1]//option[3]'}])
            self.driver.find_element(
                var_loc_keys[0],
                var_loc_keys[1]).click()

            var_loc_keys = self.mng.get_locator([{'xpath': '//div[3]/form/select[2]//option[6]'}])
            self.driver.find_element(
                var_loc_keys[0],
                var_loc_keys[1]).click()
            ...
            source = self.mng.get_locator([{'id': 'address'}])

            target = self.mng.get_locator([{'name': 'toPort'}])
            ...
            var_loc_keys = self.mng.get_locator([{'linktext': 'destination of the week! The Beach!'}])
            self.driver.find_element(
                var_loc_keys[0],
                var_loc_keys[1]).click()
            ...
            var_loc_as = self.mng.get_locator([{'xpath': "//*[@id='basics']/h2"}])

            self.vars['Basic'] = self.driver.find_element(
                var_loc_as[0],
                var_loc_as[1]).get_attribute('innerText')

            var_loc_as = self.mng.get_locator([{'xpath': "//*[@id='basics']/h1"}])
