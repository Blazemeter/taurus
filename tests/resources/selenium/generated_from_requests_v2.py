...
class TestLocSc(unittest.TestCase):
    ...
    def _1_Test_V2(self):
        with apiritif.smart_transaction('Test V2'):
            ...
            var_loc_chain = self.loc_mng.get_locator([{'id': 'invalid_id'}, {'xpath': self.vars['my_xpath_locator']}])
            ActionChains(self.driver).click_and_hold(self.driver.find_element(
                var_loc_chain[0],
                var_loc_chain[1])).perform()

            var_loc_chain = self.loc_mng.get_locator([{'id': 'id_123'}])
            ActionChains(self.driver).move_to_element_with_offset(self.driver.find_element(
                var_loc_chain[0],
                var_loc_chain[1]), -10, -10).perform()

            var_loc_chain = self.loc_mng.get_locator([{'name': 'name_123'}])
            ActionChains(self.driver).move_to_element(self.driver.find_element(
                var_loc_chain[0],
                var_loc_chain[1])).perform()

            source = self.loc_mng.get_locator([{'name': 'invalid_name'}, {'xpath': '/html/body/div[2]/div/p[2]/a'}])

            target = self.loc_mng.get_locator([{'css': 'invalid_css'}, {'xpath': '/html/body/div[3]/form/div'}])
            ...
            var_loc_as = self.loc_mng.get_locator([{'css': 'myclass'}, {'xpath': '/html/body/div[3]/h2'}])
            self.assertEqual(self.driver.find_element(
                var_loc_as[0],
                var_loc_as[1]).get_attribute('innerText').strip(), 'Choose your departure city:'.strip())

            var_loc_as = self.loc_mng.get_locator([{'css': 'myclass'}, {'xpath': '/html/body/div[3]/form/div/input'}])
            self.assertEqual(self.driver.find_element(
                var_loc_as[0],
            ...
            var_loc_as = self.loc_mng.get_locator([{'xpath': '/html/body/div[3]/h2'}])
            ...
            var_loc_keys = self.loc_mng.get_locator(
                [{'xpath': '/wrong/one'}, {'xpath': '/html/body/div[3]/form/div/input'}])
            self.driver.find_element(
                var_loc_keys[0],
                var_loc_keys[1]).click()

            var_loc_keys = self.loc_mng.get_locator([{'xpath': '/doc/abc'}, {
                'css': 'body > div.container > table > tbody > tr:nth-child(1) > td:nth-child(2) > input'}])
            self.driver.find_element(
                var_loc_keys[0],
                var_loc_keys[1]).send_keys(Keys.ENTER)

            var_loc_keys = self.loc_mng.get_locator([{'id': 'fjkafjk'}, {'css': 'testCss'}])
            self.driver.find_element(
                var_loc_keys[0],
            ...
            var_loc_select = self.loc_mng.get_locator([{'css': 'myclass'}, {'xpath': '//*[@id="cardType"]'}])
            ...
            var_loc_wait = self.loc_mng.get_locator([{'css': 'invalid_css'}, {'name': 'inputName'}])
