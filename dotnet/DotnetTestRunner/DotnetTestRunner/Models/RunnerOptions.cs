...
namespace DotnetTestRunner.Models
{
    ...
    {
        public static Option IterationOption { get; set; } = new Option("--iterations")
        {
            Description = "Number of iterations over test suite to make.",
            Argument = new Argument<int>(() => 0)
        };

        public static Option DurationOption { get; set; } = new Option("--duration")
        {
            Description = "Duration limit of test suite execution.",
            Argument = new Argument<int>(() => 0)
        };

        public static Option ConcurrencyOption { get; set; } = new Option("--concurrency")
        {
            Description = "Number of concurrent users.",
            Argument = new Argument<int>(() => 1)
        };

        public static Option RampUpOption { get; set; } = new Option("--ramp-up")
        {
            Description = "Time to ramp all concurrent users.",
            Argument = new Argument<int>("", () => 0)
        };

        public static Option ReportFileOption { get; set; } = new Option("--report-file")
        {
            Description = "Name of report file.",
            Argument = new Argument<string>(() => "report.ldjson")
        };

        public static Option TargetAssemblyOption { get; set; } = new Option("--target")
        {
