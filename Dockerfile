...
RUN bzt -install-tools -v && bzt /tmp/bzt-src/examples/all-executors.yml -o settings.artifacts-dir=/tmp/all-executors-artifacts -sequential || (ls -lh /tmp/all-executors-artifacts ; cat /tmp/all-executors-artifacts/geckodriver.log; cat /tmp/all-executors-artifacts/webdriver.log; cat /tmp/all-executors-artifacts/nose-1.err; exit 1)
